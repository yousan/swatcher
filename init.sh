...
curl $SWATCH_CONF_URL | tee $SWATCH_CONF_DIR/secure.conf
...
curl $ALERT_SCRIPT_URL | tee /usr/bin/authAlert.sh
...
if [[ $DISTRIBUTION =~ Ubuntu  ]]; then
    curl $SWATCH_CRON_UBUNTU_URL | tee /etc/cron.d/swatchron
elif [[ $DISTRIBUTION =~ CentOS ]]; then
    curl $SWATCH_CRON_CENTOS_URL | tee /etc/cron.d/swatchron
else
